...
PROMPTS="${SKILLSBOOK_PROMPTS:-/data/prompts}"
SEED="${SKILLSBOOK_SEED_DIR:-/opt/skillsbook/skills-semilla}"

for CARPETA in "$LIB" "$PROMPTS"; do
  if ! mkdir -p "$CARPETA" 2>/dev/null || [ ! -w "$CARPETA" ]; then
    echo "ERROR: no puedo escribir en $CARPETA." >&2
